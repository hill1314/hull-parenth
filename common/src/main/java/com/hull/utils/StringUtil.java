...
//            String webRoot = App.getAppPath();
     ...
     * @param variableValues 含变量的键值对
     * @return 返回替换后的SQL
     * @thorws SystemException 对应的SQL参数为Null时，反馈异常
     */
    public static String replaceDftChkNullVariable(String variableSql,
                                                   Map<String, Object> variableValues) throws Exception {
        // 缓存字符区
        StringBuffer sb = new StringBuffer();
    ...
    public static Long[] convertLongArray(String[] sArray) {
        ...
        //Long[] prod_inst_id_l = (Long[]) ConvertUtils.convert(sArray, Long.class);
     ...
     * String sql = "select * from cust where custId = :custId and custName = :custName";
     * Map<String, Object> map = new HashMap<String, Object>();
     * List<String,String> pamaList = new ArrayList<String,String>();
     * pamaList.add("123123");
     * pamaList.add("客户");
     * map.put("pamaList",pamaList);
     * map.put("sql",sql);
     * System.out.println(replaceDftVariable(sql, map));
     * 输出：
     * select * from cust where custId = ? and custName = ?
     * pamaList
     * </pre>
     *
     * @author chanwj
     ...
     * @thorws SystemException 对应的SQL参数为Null时，反馈异常
     */
    public static Map<String,Object> replaceVariable(String variableSql,
                                                     Map<String, Object> variableValues) throws SystemException {
        Map<String,Object> retMap = new HashMap<String,Object>();
        // 缓存字符区
        StringBuffer sb = new StringBuffer();
        List<String> pamaList = new ArrayList<String>();
        // 先对 inXML 进行seq 占位符替换
        Matcher matcher = null;
        matcher = DEFAULT_VARIABLE_PATTERN.matcher(variableSql);
		while (matcher.find()) {
			...
			Object keyValue = variableValues.get(variableName);
			//判断如果是拼年月参数，直接替换了
			int index_ = variableSql.indexOf(":"+variableName);
			String index_s = variableSql.substring(index_-1,index_);
			if(StringUtil.isEqual("_",index_s)){
				matcher.appendReplacement(sb, StringUtil.obj2Str(keyValue));
				continue;
			}
			...
			if (StringUtil.isNotEmptyOrNull(keyValue)) {
				if (StringUtil.obj2Str(keyValue).indexOf(",") > 0) {
					isSpecial = true;
					String[] keys = StringUtil.obj2Str(keyValue).split(",");
					for (String kItem : keys) {
						variableValue = "?," + variableValue;
						pamaList.add(kItem);
					}
				}
			...
			if (!isSpecial) {
				variableValue = "?";
				pamaList.add(StringUtil.obj2Str(keyValue));
			} else {
